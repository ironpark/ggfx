...
func (u *UserInterface) KeyName(key Key) string {
	...
	if u.keyboardLayoutMap.IsUndefined() {
		// TODO: The layout map is fetched once and then cached for the process's life, so a
		// keyboard change after the first KeyName is not picked up. It used to be reset every
		// tick, by the input snapshot that no longer exists.
